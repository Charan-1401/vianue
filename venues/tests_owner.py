...
class OwnerVenueDashboardTests(APITestCase):
    ...
    def test_venue_public_profile_endpoint_returns_media(self):
        photo = SimpleUploadedFile('front.jpg', b'photo-bytes', content_type='image/jpeg')
        video = SimpleUploadedFile('walkthrough.mp4', b'video-bytes', content_type='video/mp4')
        ...
        public_response = self.client.get(f'/api/venues/{venue.id}/profile/')
        self.assertEqual(public_response.status_code, 200)
        self.assertEqual(public_response.data['id'], venue.id)
        self.assertEqual(public_response.data['name'], 'Lakeview Hall')
        self.assertEqual(len(public_response.data['media']), 2)
